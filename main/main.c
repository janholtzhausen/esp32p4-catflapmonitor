...
static esp_err_t capture_video_image_upload(web_cam_video_t *video, bool is_jpeg)
{
    esp_err_t ret = ESP_OK;
    struct v4l2_buffer buf;
    ...
    if (!is_jpeg || video->pixel_format == V4L2_PIX_FMT_JPEG) {
        const uint8_t *p = (const uint8_t *)video->buffer[buf.index];
        jpeg_encoded_size = buf.bytesused;

        ret = post_jpeg_bytes(p, jpeg_encoded_size);
        if (ret != ESP_OK) goto out_qbuf;
    } else {
        ESP_GOTO_ON_FALSE(xSemaphoreTake(video->sem, portMAX_DELAY) == pdPASS,
                          ESP_FAIL, out_qbuf, TAG, "failed to take semaphore");
        ...
        ret = post_jpeg_bytes((const uint8_t *)video->jpeg_out_buf, jpeg_encoded_size);
        if (ret != ESP_OK) goto out_qbuf;
    }
...
static esp_err_t capture_image_handler(httpd_req_t *req)
{
    ...
    esp_err_t err = capture_video_image_upload(&web_cam->video[desc.index], true);

    // Force "not a file" semantics as much as the server can
    httpd_resp_set_hdr(req, "Cache-Control", "no-store");
    httpd_resp_set_hdr(req, "Content-Disposition", "inline");
    httpd_resp_set_type(req, "text/plain");

    if (err == ESP_OK) {
        return httpd_resp_send(req, "OK\n", 3);   // <-- IMPORTANT: actually send a body
    } else {
